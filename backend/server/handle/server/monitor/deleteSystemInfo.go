...
func DeleteSystemInfo(c *gin.Context) {
    ...
    `
    var existingID int
    err = tx.Raw(query, deleteSystemInfoRequest.Host_name).Scan(&existingID).Error
    if err != nil {
        log.Println("数据库查询失败")
        tx.Rollback() // 回滚事务
    ...
    if existingID > 0 {
        deleteQuery := `
            ...
            WHERE host_name = $1 
        `
        if err = tx.Exec(deleteQuery, deleteSystemInfoRequest.Host_name).Error; err != nil {
    ...
    `
    err = tx.Raw(query, deleteSystemInfoRequest.Host_name, deleteSystemInfoRequest.Host, username).Scan(&existingID).Error
    if err != nil {
    ...
    if existingID > 0 {
        deleteQuery := `
            ...
            WHERE host_name = $1 AND ip = $2 AND user_name = $3
        `
        if err = tx.Exec(deleteQuery, deleteSystemInfoRequest.Host_name, deleteSystemInfoRequest.Host, username).Error; err != nil {
